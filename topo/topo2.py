...
def link_helper(net, routers, i, j, delay='20ms'):
    ...
    Ra = routers['R%d' % a]
    Rb = routers['R%d' % b]
    net.addLink(Ra, Rb, cls=TCLink, delay=delay, bw=1, intfName1='R%d-%d%d' % (a, a, b), params1={'ip': '10.0.%d%d.%d/24' % (a, b, a)},
...
def build_topo():
    ...
    link_helper(net, routers, 1, 2, delay='15ms')
    link_helper(net, routers, 1, 3)
    link_helper(net, routers, 1, 4)
    link_helper(net, routers, 2, 5)
    link_helper(net, routers, 3, 5)
    link_helper(net, routers, 4, 5)
